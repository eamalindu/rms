...
public interface BatchDAO extends JpaRepository<Batch,Integer> {
    ...
    @Query("SELECT b FROM Batch b WHERE b.courseID.id =?1 and b.isWeekday = true and (b.batchStatusID.id=1 or b.batchStatusID.id=2) and b.commenceDate >= curdate() ")
    List<Batch> getActiveWeekDayBatchesByCourseId(Integer courseId);

    @Query("SELECT b FROM Batch b WHERE b.courseID.id =?1 and b.isWeekday = false and (b.batchStatusID.id=1 or b.batchStatusID.id=2) and b.commenceDate >= curdate()")
