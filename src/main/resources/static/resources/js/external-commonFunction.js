...
const createInquiryRadioCards = (dataList,functionEx,container)=>{
    if(dataList!=="") {
        container.innerHTML = "<p class='text-muted'>Results From Inquiry Database</p>"; // Clear previous content
        rowInq = document.createElement("div");
        rowInq.className = "row p-2";
        container.appendChild(rowInq);
        ...
        cardDiv.innerHTML = `
          <div class="card-body p-1">
            ...
            <div class="w-100 d-flex align-items-center justify-content-center">
                <div class="">
                <h6 class="fw-bold text-steam-green m-0">${dataList.firstName} ${dataList.lastName}</h6>
                <p class="small m-0 text-start fw-bold"><small>${dataList.inquiryNumber}</small></p>
                <p class="small m-0 text-muted text-start"><small><i class="fa fa-phone-alt fa-sm"></i> ${dataList.primaryMobileNumber}</small></p>
                <p class="small m-0 text-muted text-start"><small><i class="fa fa-envelope fa-sm"></i>  ${dataList.email == null ? 'n/a' : dataList.email}</small></p>
            </div>
            </div>
          `;
        ...
        col.appendChild(cardDiv);
        rowInq.appendChild(col);
    }else{
        container.innerHTML = '<p class="text-red text-center small">No Records Found For <b>'+studentSearchID.value+'</b> in the Exsisting Inquiry Database! </p>';
